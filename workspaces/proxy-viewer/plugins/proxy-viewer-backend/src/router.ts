...
function convertHeaders(headers: Record<string, string | undefined> | undefined): { [key: string]: string | undefined } | undefined {
  if (!headers) {
    return undefined;
  }
...
function convertEndpoint(path: string, endpoint: string | ProxyEndpointConfig): ParsedProxyEndpoint {
...
}: // httpAuth,
{
  ...
  router.get('/endpoints', async (_req, res) => {
    const proxyViewerConfig = rootConfig.getOptional<ProxyViewerConfig>('proxyViewer');
    if (!proxyViewerConfig || !proxyViewerConfig.dangerouslyAnyoneCanReadAllProxies) {
    ...
    if (proxyConfig?.endpoints) {
      const endpoints = Object.entries(proxyConfig.endpoints)
        .map(([path, endpoint]) => convertEndpoint(path, endpoint));
      res.json(endpoints);
    } else if (typeof proxyConfig?.proxy === 'object') {
      const endpoints = Object.entries(proxyConfig as Record<string, string | ProxyEndpointConfig>)
