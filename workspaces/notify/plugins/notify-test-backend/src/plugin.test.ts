...
import { notifyTestPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [notifyTestPlugin],
    });

    await request(server).get('/api/notify-test/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/notify-test/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/notify-test/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/notify-test/todos/${createdTodoItem.id}`)
